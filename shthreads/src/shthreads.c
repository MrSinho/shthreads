...
ShThreadsStatus shAllocateThreads(
...
ShThreadsStatus shCreateThread(
    uint32_t      idx, 
    void*         p_func, 
    uint32_t      stack_size, 
    ShThreadPool* p_pool
...
ShThreadsStatus shLaunchThreads(
...
ShThreadsStatus shGetThreadState(
    uint32_t       thread_idx, 
...
ShThreadsStatus shWaitForThreads(
    uint32_t      first_thread, 
    uint32_t      thread_count, 
    uint64_t      ms_timeout, 
...
ShThreadsStatus shExitCurrentThread(
...
ShThreadsStatus shReleaseThreads(
    ShThreadPool* p_pool
) {
    shThreadsError(p_pool            == NULL, "invalid thread pool memory",    return SH_INVALID_THREAD_POOL_MEMORY);
...
ShThreadsStatus shWaitForMutexes(
    ...
    for (uint32_t mutex_idx = first_mutex; mutex_idx < (first_mutex + mutex_count); mutex_idx++) {
        lock_status += pthread_mutex_lock(&p_mutexes[mutex_idx]) == 0;
    }
...
ShThreadsStatus shUnlockMutexes(
    uint32_t first_mutex, 
    uint32_t mutex_count, 
    ShMutex* p_mutexes
) {
    ...
    uint32_t unlock_status = 0;
    for (uint32_t mutex_idx = first_mutex; mutex_idx < (first_mutex + mutex_count); mutex_idx++) {
        unlock_status += pthread_mutex_unlock(&p_mutexes[mutex_idx]) == 0;
        shThreadsError(unlock_status != mutex_count, "failed unlocking mutexes", return SH_MUTEX_UNLOCK_FAILURE);
    }
#endif//_WIN32
...
ShThreadsStatus shReleaseMutexes(
    uint32_t first_mutex, 
    uint32_t mutex_count, 
    ShMutex* p_mutexes
) {
...
#ifdef _WIN32
    for (uint32_t mutex_idx = 0; mutex_idx < mutex_count; mutex_idx++) {
        CloseHandle(p_mutexes[mutex_idx]);
    }
#else
    for (uint32_t mutex_idx = 0; mutex_idx < mutex_count; mutex_idx++) {
        pthread_mutex_destroy(&p_mutexes[mutex_idx]);
    }
#endif//_WIN32
